...
DROP TABLE user_survey_answers
  question_id INTEGER REFERENCES questions(id), -- Seçeneğin bağlı olduğu sorunun kimlik numarası
...
DROP COLUMN answers,
ADD COLUMN question_option_id INTEGER REFERENCES question_options(id);
...
INSERT INTO surveys (survey_name, admin_id) VALUES ('NEÜ Çoktan Seçmeli Anket Sorulari2', 5);
...
SELECT*FROM user_survey_answers
INSERT INTO user_survey_answers (user_id, survey_id, question_id, answers)
VALUES 
    (1,2,6,'a');
...
------------------
INSERT INTO questions (survey_id, question_text, question_type)
VALUES (3, 'Ders Geçme notu kaçtır?', 'Açık Uçlu');
select*from questions

INSERT INTO questions (survey_id, question_text, question_type)
VALUES (3, 'ALES sonucu kaç yıl geçerlidir?', 'Açık Uçlu');
...
-- Large Object modülünü etkinleştir
CREATE EXTENSION lo;

-- Dosyayı yükle ve OID'yi al
SELECT lo_import('A:\Videolar\yuklenecekVideo.mp4') AS oid;

-- OID'yi kullanarak veritabanına ekle
-- Dosyayı yükle ve OID'yi al
DO $$
DECLARE
    file_oid OID;
BEGIN
    SELECT lo_import('A:\Videolar\yuklenecekVideo.mp4') INTO file_oid;
    INSERT INTO video (name, content) VALUES ('yuklenecekVideo.mp4', file_oid);
END $$;


-- OID'yi kullanarak büyük nesneyi al
SELECT lo_export(content, 'A:\Videolar\yuklenecekVideo_cikti.mp4') FROM video WHERE id = 1;
